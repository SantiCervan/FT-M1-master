...
Node.prototype.balance = function(anterior, arbol, from) {
    ...
    if ((thisbf < 0 && this.maxBalanceFactor() >= abs(thisbf))) {
        ...
        if (this.right)
            if (!this.right.isBalanced()) return this.right.balance(this, arbol, 'right');
        if (this.left)
            if (!this.left.isBalanced()) return this.left.balance(this, arbol, 'right');
            //este arbol esta desbalanceado
        if (this == arbol) { return this.rotate() } else { anterior[from] = this.rotate(); }
    }
    return arbol;
